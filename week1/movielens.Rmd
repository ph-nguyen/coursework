...
```
>>>>>>> a6ef8d2ffa43bd55c395c04f3cf98c7b09cb457d:week1/movielens.Rmd
...
ratings %>% 
  group_by(movie_id) %>% 
  summarise(num_rating = n()) %>%
  ggplot(aes(x=movie_id, y=num_rating)) +
  geom_point() +
...
ratings %>% 
  ...
  ggplot(aes(x=movie_id, y= mean_rating)) +
  ?geom_density() +
...
ratings %>% group_by(movie_id) %>% 
  summarise(num_rating = n()) %>% 
  mutate(movie_rank= rank(num_rating, ties.method = c("first"))) %>%
  arrange(movie_rank)
...
ratings %>% group_by(user_id) %>%
  ...
  ggplot(aes(x=user_id, y=num_rating)) +
  geom_point()
